...
def instruction_menu(key=None):
    from telegram import CopyTextButton
    keyboard = []
    if key:
        keyboard.append([InlineKeyboardButton("🔑 Скопировать ключ", copy_text=CopyTextButton(text=key), style="success")])
    
    keyboard.append([InlineKeyboardButton("🔙 Назад", callback_data="subscription_mgmt")])
    return InlineKeyboardMarkup(keyboard)
